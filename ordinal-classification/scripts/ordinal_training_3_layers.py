...
# df["embedding"] = df.embedding.apply(literal_eval).apply(np.array)  # type: ignore
...
# Define the neural network model for CORN
class MultilayerPerceptron(nn.Module):
    def __init__(
        self, input_dim, n_classes, num_hidden_1, num_hidden_2, num_hidden_3, dropout_rate=0.5
    ):
        super().__init__()
        ...
        self.predictor = nn.Sequential(
            ...
            nn.Dropout(dropout_rate),
            nn.Linear(num_hidden_3, (n_classes - 1)),
        )
...
def objective(trial):
    ...
    num_hidden_3 = trial.suggest_int("num_hidden_3", 16, 128)
    ...
    for _ in range(num_epochs):
        ...
        for features, targets in train_loader:
            ...
            # CORN loss
            loss = loss_corn(logits, targets, model.n_classes)
...
params = {
    ...
    "num_hidden_3": best_params["num_hidden_3"],
